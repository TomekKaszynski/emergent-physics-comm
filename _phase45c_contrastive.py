def run_phase45c_contrastive_perception():
    """Phase 45c: Contrastive Slot Attention on CLEVRER — InfoNCE temporal loss.

    Replace MSE temporal loss (dead gradients) with InfoNCE contrastive loss.
    Cross-video negatives give strong gradients even between similar slots.
    Based on SlotContrast (CVPR 2025, Manasyan et al.).
    """
    import time
    import json
    import os
    ...
    print("=" * 70, flush=True)
    print("PHASE 45c: Contrastive Slot Attention on CLEVRER", flush=True)
    print("  InfoNCE contrastive temporal loss + 100 videos", flush=True)
    print("=" * 70, flush=True)
    t0 = time.time()
    ...
    print(f"│  Contrastive: α={alpha_contrastive} (warmup {alpha_warmup_epochs}ep), "
          f"τ={temperature}, stride={pair_stride}, InfoNCE", flush=True)
    ...
    for epoch in range(1, sa_epochs + 1):
        ...
        for pairs in batches:
            ...
            total_loss = recon_loss + alpha_eff * ctr_loss
    ...
    summary = (
        f"Phase 45c: Contrastive Slot Attention\n\n"
        f"Data: {n_videos} videos × {n_frames} frames\n"
        f"Model: DINOv2 → SA (7 slots, InfoNCE)\n"
        f"       α={alpha_contrastive}, τ={temperature}\n\n"
        f"                     45c     45    45b\n"
    ...
    fig.suptitle(f'Phase 45c: Contrastive Slot Attention on CLEVRER\n'
                 f'err={mean_tracking_error*100:.1f}% '
                 f'consistency={mean_consistency:.0f}% '
                 f'binding={mean_binding:.0f}% | {verdict}',
                 fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig(str(OUTPUT_DIR / "phase45c_contrastive_perception.png"),
                dpi=150, bbox_inches='tight')
    plt.close()
    print(f"│  Saved results/phase45c_contrastive_perception.png", flush=True)
