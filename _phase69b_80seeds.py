"""
Phase 69b: 80-seed compositionality characterization
=====================================================
...
square holdout) but with 80 seeds for statistical power.

Run:
  PYTHONUNBUFFERED=1 PYTORCH_ENABLE_MPS_FALLBACK=1 python3 _phase69b_80seeds.py
"""
...
def main():
    print("=" * 70, flush=True)
    print("Phase 69b: 80-seed compositionality characterization", flush=True)
    print("=" * 70, flush=True)
    ...
    # ════════════════════════════════════════════════════════════
    print(f"\n\n{'='*70}", flush=True)
    print(f"FULL TABLE: Phase 69b (80 seeds)", flush=True)
    ...
    if len(comp_hb) > 1 and len(noncomp_hb) > 1:
        ...
        cohens_d = (comp_hb.mean() - noncomp_hb.mean()) / np.sqrt((comp_hb.std()**2 + noncomp_hb.std()**2) / 2)
        ...
        print(f"      Effect size (Cohen's d): {cohens_d:.2f}", flush=True)
    ...
    save_path = RESULTS_DIR / "phase69b_80seeds.json"
