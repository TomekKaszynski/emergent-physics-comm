...
=======================================================
Sender communicates with 3 receivers simultaneously.
Loss = average BCE across all receivers → universally decodable messages.
Staggered IL: every 40 epochs, reset ONE receiver (cycling through them).
Multi-seed: runs 5 seeds and prints summary table.

Run:
  PYTHONUNBUFFERED=1 PYTORCH_ENABLE_MPS_FALLBACK=1 python3 _phase54d_population.py
"""
...
def train_population(sender, receivers, data_t, e_bins, f_bins,
    ...
    """Train sender against population of receivers with staggered IL."""
    s_lr = SENDER_LR
    r_lr = RECEIVER_LR
    ...
    for epoch in range(COMM_EPOCHS):
        # Staggered IL: reset one receiver at a time, cycling
        if epoch > 0 and epoch % RECEIVER_RESET_INTERVAL == 0:
            reset_idx = (epoch // RECEIVER_RESET_INTERVAL - 1) % len(receivers)
            receivers[reset_idx] = CompositionalReceiver(msg_dim, HIDDEN_DIM).to(device)
            receiver_opts[reset_idx] = torch.optim.Adam(
                receivers[reset_idx].parameters(), lr=r_lr)
            gen = epoch // RECEIVER_RESET_INTERVAL
            print(f"    ** Reset receiver {reset_idx} at epoch {epoch+1} "
...
def run_single_seed(seed, data_t, e_bins, f_bins, train_ids, holdout_ids, device):
    ...
    print(f"    Training sender vs {N_RECEIVERS} receivers "
          f"(IL={RECEIVER_RESET_INTERVAL}, staggered)...", flush=True)
...
def main():
    print("=" * 70, flush=True)
    print("Phase 54d: Population-based Compositional Communication", flush=True)
    print("=" * 70, flush=True)
    print(f"  Device: {DEVICE}", flush=True)
    print(f"  Receivers: {N_RECEIVERS}", flush=True)
    print(f"  Staggered IL: reset 1/{N_RECEIVERS} every {RECEIVER_RESET_INTERVAL} epochs",
          flush=True)
    ...
    # ════════════════════════════════════════════════════════════
    print(f"\n\n{'='*70}", flush=True)
    print("SUMMARY: Phase 54d Population-based (3 receivers, staggered IL=40)", flush=True)
    ...
    print(f"\n  COMPARISON with IL=40 baseline (original script ×5):", flush=True)
    print(f"  {'Condition':<25} | {'Holdout Both':<16} | {'PosDis':<16} | "
          f"{'MI→e':<10} | {'MI→f':<10}", flush=True)
    print(f"  {'─'*25}-+-{'─'*16}-+-{'─'*16}-+-{'─'*10}-+-{'─'*10}", flush=True)
    ...
    # IL=40 baseline from full comparison
    bl_hb = [0.781, 0.795, 0.846, 0.688, 0.743]
    bl_pd = [0.344, 0.219, 0.429, 0.157, 0.304]
    bl_me = [0.986, 0.736, 0.914, 0.886, 0.962]
    bl_mf = [0.783, 1.012, 1.106, 0.796, 0.806]

    print(f"  {'IL=40 (single rx)':<25} | {fmt(bl_hb, pct=True):<16} | "
          f"{fmt(bl_pd):<16} | {fmt(bl_me):<10} | {fmt(bl_mf):<10}", flush=True)
    print(f"  {'Population (3 rx+IL)':<25} | {fmt(hb, pct=True):<16} | "
          f"{fmt(pd):<16} | {fmt(me):<10} | {fmt(mf):<10}", flush=True)
    ...
    save_path = RESULTS_DIR / "phase54d_population.json"
