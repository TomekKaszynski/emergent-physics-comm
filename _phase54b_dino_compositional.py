...
==========================================================
Same agents, same task, same communication pressure — just better eyes.
DINOv2 ViT-S/14 replaces the small CNN as the frozen visual backbone.

Architecture:
  - DINOv2 (frozen) extracts 384-dim CLS token per frame
  - Temporal MLP pools 8 frames → 128-dim scene representation
  - Sender: temporal encoder → TWO Gumbel-Softmax heads (2×8)
  - Receiver: two output heads (elasticity, friction comparison)
  - Control: single Gumbel-Softmax head (1×64)

Dataset: same 300 ramp scenes from Phase 54 (kubric/output/ramp_dataset/)
Features pre-extracted and cached for speed.

Run:
  PYTHONUNBUFFERED=1 PYTORCH_ENABLE_MPS_FALLBACK=1 python3 _phase54b_dino_compositional.py
"""
...
def extract_dino_features(dataset_dir, cache_path, device):
    """Extract DINOv2 CLS tokens for all scenes. Returns (300, 8, 384)."""
    ...
    print(f"  DINOv2 params: {n_params:,} (all frozen)", flush=True)
    ...
    for sid in range(TOTAL_SCENES):
        ...
        for fi in frame_indices:
            ...
            # Resize to 224×224 for DINOv2 (trained on 224)
            img = img.resize((224, 224), Image.BILINEAR)
            img_np = np.array(img, dtype=np.float32) / 255.0
            frames.append(img_np)

        if len(frames) != N_FRAMES:
            print(f"  WARNING: scene_{sid:04d} missing frames, skipping", flush=True)
            continue

        # (T, H, W, 3) -> (T, 3, H, W) -> normalize -> forward
        frames_t = torch.tensor(np.stack(frames)).permute(0, 3, 1, 2).to(device)
        frames_t = (frames_t - dino_mean) / dino_std

        with torch.no_grad():
            cls_tokens = dino(frames_t)  # (T, 384) — CLS token

        all_features.append(cls_tokens.cpu())
        e_bins.append(e_bin)
        f_bins.append(f_bin)

        if (sid + 1) % 50 == 0 or sid == 0:
            elapsed = time.time() - t_start
            eta = elapsed / (sid + 1) * (TOTAL_SCENES - sid - 1)
            print(f"    [{sid+1:3d}/{TOTAL_SCENES}] {elapsed:.0f}s  ETA {eta:.0f}s",
                  flush=True)

        if device.type == 'mps' and (sid + 1) % 100 == 0:
            torch.mps.empty_cache()

    features = torch.stack(all_features)  # (N, T, 384)
    ...
    for epoch in range(COMM_EPOCHS):
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            ...
            print(f"  Ep {epoch+1:3d}: loss={epoch_loss/n_batches:.4f}  tau={tau:.2f}  "
                  ...
                  f"ent={epoch_entropy/n_batches:.2f}{nan_str}  "
...
def plot_results(history_2x8, history_1x64, comp_2x8, comp_1x64,
    ...
    ax1.set_title('2x8 Training Curves (DINOv2)')
    ...
    ax6.axis('off')
    summary_lines = [
        "Phase 54b: DINOv2 Compositional Communication",
        "=" * 44,
...
def main():
    print("=" * 60, flush=True)
    print("Phase 54b: Compositional Communication (DINOv2)", flush=True)
    print("=" * 60, flush=True)
    ...
    # ================================================================
    # STAGE 0: Extract DINOv2 features
    # ================================================================
    cache_path = str(RESULTS_DIR / "phase54b_dino_features.pt")
    data_t, e_bins, f_bins = extract_dino_features(
        DATASET_DIR, cache_path, DEVICE)
    ...
    final_results['mode'] = 'dino_pixel'

    results_path = RESULTS_DIR / "phase54b_results.json"
    with open(results_path, 'w') as f:
        json.dump(final_results, f, indent=2)
    print(f"\n  Saved results to {results_path}", flush=True)

    # Save models
    model_path = RESULTS_DIR / "phase54b_model.pt"
    torch.save({
    ...
    # Visualization
    plot_path = RESULTS_DIR / "phase54b_compositionality.png"
    plot_results(history_2x8, history_1x64, comp_2x8, comp_1x64,
                 final_results, e_bins, f_bins, plot_path)

    dt = time.time() - t_total
    print(f"\n{'='*60}", flush=True)
    print(f"Phase 54b complete! Total time: {dt/60:.1f} min", flush=True)
    print(f"{'='*60}", flush=True)
