"""
Phase 54g: 1x25 control at 400 epochs
======================================
Null hypothesis: does extended training + population + IL produce high
accuracy WITHOUT positional structure? Same setup as 54f but with a single
1x25 head instead of 2x5. PosDis = 0.0 by definition (one position).

Run:
  PYTHONUNBUFFERED=1 PYTORCH_ENABLE_MPS_FALLBACK=1 python3 _phase54g_control_extended.py
"""
...
def compute_compositionality(sender, data_t, e_bins, f_bins, device):
    ...
    # PosDis — 0.0 for single-position (can't disentangle)
    if n_pos >= 2:
        pos_dis = 0.0
        for p in range(n_pos):
...
def run_single_seed(seed, data_t, e_bins, f_bins, train_ids, holdout_ids, device):
    ...
    print(f"    Training sender (1x{VOCAB_SIZE}) vs {N_RECEIVERS} receivers "
...
def main():
    print("=" * 70, flush=True)
    print("Phase 54g: 1x25 control at 400 epochs", flush=True)
    print("=" * 70, flush=True)
    print(f"  Device: {DEVICE}", flush=True)
    print(f"  Vocab: {VOCAB_SIZE} (1x{VOCAB_SIZE} control — no positional structure)", flush=True)
    ...
    # ════════════════════════════════════════════════════════════
    print(f"\n\n{'='*70}", flush=True)
    print(f"FULL TABLE: Phase 54g control (1x25, 400 epochs, 20 seeds)", flush=True)
    ...
    # Comparison summary
    # ════════════════════════════════════════════════════════════
    print(f"\n  CONTROL SUMMARY:", flush=True)
    print(f"  PosDis = 0.000 for all seeds (1 position, can't disentangle)", flush=True)
    print(f"  Mean TopSim: {np.mean(ts):.3f} +/- {np.std(ts):.3f}", flush=True)
    print(f"  Mean Holdout: {np.mean(hb):.1%} +/- {np.std(hb):.1%}", flush=True)
    print(f"  Mean MI->e: {np.mean(me):.3f} +/- {np.std(me):.3f}", flush=True)
    print(f"  Mean MI->f: {np.mean(mf):.3f} +/- {np.std(mf):.3f}", flush=True)

    # TopSim histogram
    print(f"\n  TopSim HISTOGRAM:", flush=True)
    ...
    for i in range(len(bin_edges) - 1):
        count = sum(1 for t in ts if bin_edges[i] <= t < bin_edges[i + 1])
        bar = "#" * (count * 3)
        label = bin_labels[i]
    ...
    save_path = RESULTS_DIR / "phase54g_control.json"
