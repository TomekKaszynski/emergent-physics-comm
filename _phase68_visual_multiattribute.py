...
=============================================================================
Scales Phase 67 from 2 to 6 continuous visual properties on CIFAR-100.
Phase 67 showed weak compositional advantage because brightness/saturation were
too easy (oracle 75.7%). More properties with varying difficulty should create
genuine bottleneck pressure where compositional structure matters.

Dataset: CIFAR-100 (60K images), DINOv2 ViT-S/14 features (cached from Phase 66)
Properties (from raw pixels):
  1. Brightness    — mean luminance
  2. Saturation    — mean HSV saturation
  3. Hue conc.     — circular mean resultant length of hue
  4. Edge density   — fraction of Sobel-edge pixels
  5. Spatial freq.  — high-frequency energy ratio (FFT)
  6. Color diversity — unique quantized color triplets

Task: Property comparison (6 binary heads)
  - 2 agents, each sees one image's DINOv2 features
  - Receiver predicts which image has higher value for each property

Five conditions × 10 seeds:
  (a) COMP_6POS: 6 positions × vocab 5  (1 slot per property)
  (b) COMP_4POS: 4 positions × vocab 5  (fewer slots than properties)
  (c) COMP_8POS: 8 positions × vocab 5  (more slots than properties)
  (d) HOLISTIC:  1 position  × vocab 100
  (e) ORACLE:    raw features, no communication bottleneck

Run:
  PYTHONUNBUFFERED=1 PYTORCH_ENABLE_MPS_FALLBACK=1 python3 _phase68_visual_multiattribute.py
"""
...
CONDITIONS = ['comp_6pos', 'comp_4pos', 'comp_8pos', 'holistic', 'oracle']

CONDITION_CONFIGS = {
    'comp_6pos': {'n_positions': 6, 'vocab_size': 5},
    'comp_4pos': {'n_positions': 4, 'vocab_size': 5},
    'comp_8pos': {'n_positions': 8, 'vocab_size': 5},
    'holistic':  {'n_positions': 1, 'vocab_size': 100},
}
        ...
        for _ in range(N_BATCHES_PER_EPOCH):
            ...
            for r in receivers:
                ...
                for prop_idx in range(N_PROPERTIES):
                    ...
                    r_loss = r_loss + F.binary_cross_entropy_with_logits(
...
def main():
    print("=" * 70, flush=True)
    print("Phase 68: 6-Property Visual Attributes — Scaling Compositional Comm",
          flush=True)
    print("=" * 70, flush=True)
    ...
    # ── Trained conditions ──
    trained_conditions = ['comp_6pos', 'comp_4pos', 'comp_8pos', 'holistic']
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"[{ci+1}/{len(CONDITIONS)}] Condition: ORACLE (raw features)", flush=True)
    print(f"{'='*60}", flush=True)
    ...
    # MI specialization (COMP_6POS)
    if 'comp_6pos' in all_results and 'sender_specs' in all_results['comp_6pos']:
        print(f"\nCOMP_6POS MI Matrix (avg across seeds, sender_0):", flush=True)
        sp = all_results['comp_6pos']['sender_specs']['sender_0']
    ...
    if 'comp_6pos' in all_results and 'sender_specs' in all_results['comp_6pos']:
        sp0 = all_results['comp_6pos']['sender_specs']['sender_0']
        sp1 = all_results['comp_6pos']['sender_specs']['sender_1']
    ...
    for cname in ['comp_6pos', 'comp_4pos', 'comp_8pos', 'holistic']:
    ...
    print(f"\nCross-domain comparison (holdout all-correct):", flush=True)
    print(f"  Phase 62 (ramp 2-prop):       ~92%", flush=True)
    print(f"  Phase 64 (scenes 2-prop):     ~87%", flush=True)
    print(f"  Phase 65 (temporal 2-prop):   ~88%", flush=True)
    print(f"  Phase 67 (vision 2-prop):      76%", flush=True)
    c6 = all_results['comp_6pos']['holdout_all_correct_mean']
    print(f"  Phase 68 (vision 6-prop):     {c6:.1%}", flush=True)

    # Key comparisons
    best_comp = max(
        all_results['comp_6pos']['holdout_all_correct_mean'],
        all_results['comp_4pos']['holdout_all_correct_mean'],
        all_results['comp_8pos']['holdout_all_correct_mean'],
    )
    hol = all_results['holistic']['holdout_all_correct_mean']
    orc_all = all_results['oracle']['holdout_all_correct_mean']
    print(f"\nKey comparisons:", flush=True)
    print(f"  Best compositional: {best_comp:.1%}", flush=True)
    print(f"  Holistic:           {hol:.1%}", flush=True)
    print(f"  Oracle (ceiling):   {orc_all:.1%}", flush=True)
    print(f"  Comp advantage:     {best_comp - hol:+.1%}", flush=True)
    if orc_all > 0.01:
        print(f"  Ceiling captured:   {best_comp / orc_all:.1%}", flush=True)
    ...
    out_path = RESULTS_DIR / "phase68_visual_multiattribute.json"
