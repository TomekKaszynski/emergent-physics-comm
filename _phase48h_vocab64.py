def run_phase48h_vocab64():
    """Phase 48h: Single token vocab=64 (was 8 in 48e).

    Identical to 48e except vocab_size = 64. Single Gumbel-Softmax token,
    single-object Agent B, GT trajectories.
    """
    import time
    import json
    import os
    ...
    print("=" * 70, flush=True)
    print("PHASE 48h: Single Token vocab=64 — 1000 CLEVRER Videos", flush=True)
    print("  Same as 48e but vocab_size=64 (was 8)", flush=True)
    print("=" * 70, flush=True)
    t0 = time.time()
    ...
    vocab_size = 64  # CHANGE: was 8 in 48e
    hidden_dim = 128
    comm_epochs = 200
    comm_lr = 3e-4
    comm_batch = 64
    ...
    print(f"│  Vocab size: {vocab_size} (was 8 in 48e)", flush=True)
    ...
    }, str(OUTPUT_DIR / "phase48h_model.pt"))
    ...
    for d in range(n_direction_bins):
        if msg_by_dir[d]:
            ...
            top3_str = ", ".join([f"msg{t}:{c[t]}" for t in top3])
            print(f"│    {dir_names[d]:2s}: dominant=msg{dominant} ({cons:.0%}), "
                  f"top3=[{top3_str}]", flush=True)
    ...
    ax.set_title('Post-Collision Direction (Vocab=64)', fontsize=11)
    ...
    # Panel 2: Message-direction heatmap (top 16 most used symbols)
    ax = axes[0, 1]
    top_symbols = np.argsort(counts)[::-1][:16]
    msg_matrix = np.zeros((n_direction_bins, 16))
    for d in range(n_direction_bins):
        if msg_by_dir[d]:
            c = np.bincount(msg_by_dir[d], minlength=vocab_size).astype(float)
            c_top = c[top_symbols]
            total = c.sum()
            msg_matrix[d] = c_top / max(total, 1)
    im = ax.imshow(msg_matrix, aspect='auto', cmap='YlOrRd', vmin=0, vmax=1)
    ax.set_xlabel('Message Symbol (top 16)')
    ax.set_ylabel('True Direction')
    ax.set_yticks(range(n_direction_bins))
    ax.set_yticklabels(dir_names)
    ax.set_xticks(range(16))
    ax.set_xticklabels([str(s) for s in top_symbols], fontsize=6)
    ax.set_title('Message Usage by Direction (top 16)', fontsize=11)
    plt.colorbar(im, ax=ax, label='Frequency')
    for d in range(n_direction_bins):
        for mi, m in enumerate(top_symbols):
            if msg_matrix[d, mi] > 0.1:
                ax.text(mi, d, f'{msg_matrix[d, mi]:.1f}',
                        ha='center', va='center', fontsize=7,
                        color='white' if msg_matrix[d, mi] > 0.5 else 'black')
    ...
    summary = (
        f"Phase 48h: Single Token vocab=64\n\n"
        f"Input: GT pos+vel+speed (B=obj A only)\n"
    ...
    fig.suptitle(f'Phase 48h: Single Token vocab=64 (1000 Videos)\n'
    ...
    plt.tight_layout()
    plt.savefig(str(OUTPUT_DIR / "phase48h_vocab64.png"),
                dpi=150, bbox_inches='tight')
    plt.close()
    print(f"│  Saved results/phase48h_vocab64.png", flush=True)
