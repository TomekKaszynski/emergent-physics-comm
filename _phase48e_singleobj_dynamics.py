def run_phase48e_singleobj_dynamics():
    """Phase 48e: Single-object Agent B for larger information gap.

    Same as 48d (GT trajectories) but Agent B sees only object A's
    pre-collision trajectory, not both objects.
    Agent A: 2 objects × 17 frames × 5 = 170 dims (unchanged)
    Agent B: 1 object × 9 frames × 5 = 45 dims (was 90 in 48d)
    """
    import time
    import json
    import os
    ...
    print("=" * 70, flush=True)
    print("PHASE 48e: Single-Object Agent B Communication — 1000 CLEVRER Videos", flush=True)
    print("  Agent B sees only object A pre-collision trajectory", flush=True)
    print("=" * 70, flush=True)
    t0 = time.time()
    ...
    }, str(OUTPUT_DIR / "phase48e_model.pt"))
    ...
    ax.set_title('Post-Collision Direction (Single-Object Agent B)', fontsize=11)
    ...
    summary = (
        f"Phase 48e: Single-Obj Agent B\n\n"
    ...
    fig.suptitle(f'Phase 48e: Single-Object Agent B (1000 Videos)\n'
    ...
    plt.tight_layout()
    plt.savefig(str(OUTPUT_DIR / "phase48e_singleobj_dynamics.png"),
                dpi=150, bbox_inches='tight')
    plt.close()
    print(f"│  Saved results/phase48e_singleobj_dynamics.png", flush=True)
