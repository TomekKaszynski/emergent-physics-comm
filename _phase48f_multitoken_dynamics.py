def run_phase48f_multitoken_dynamics():
    """Phase 48f: Multi-token messages for higher bandwidth communication.

    Same as 48e (single-object Agent B, GT trajectories) but sender
    produces 4-token sequence (vocab=16 each) instead of 1 token (vocab=8).
    16^4 = 65,536 possible messages vs 8 before.
    """
    import time
    import json
    import os
    ...
    print("=" * 70, flush=True)
    print("PHASE 48f: Multi-Token Communication — 1000 CLEVRER Videos", flush=True)
    print("  4 tokens x vocab 16 = 65536 possible messages", flush=True)
    print("=" * 70, flush=True)
    t0 = time.time()
    ...
    }, str(OUTPUT_DIR / "phase48f_model.pt"))
    ...
    ax.set_title('Post-Collision Direction (Multi-Token Messages)', fontsize=11)
    ...
    fig.suptitle(f'Phase 48f: Multi-Token Communication (1000 Videos)\n'
    ...
    plt.tight_layout()
    plt.savefig(str(OUTPUT_DIR / "phase48f_multitoken.png"),
                dpi=150, bbox_inches='tight')
    plt.close()
    print(f"│  Saved results/phase48f_multitoken.png", flush=True)
