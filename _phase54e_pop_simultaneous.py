...
=====================================================
Two changes from 54d:
  1. Reset ALL 3 receivers simultaneously every 40 epochs (not staggered).
     No survivors — sender's language must be learnable from scratch by
     3 independent new listeners. Maximum learnability pressure.
  2. Vocab=5 (matching 5 property levels). With vocab=8 there's room for
     holistic shortcuts; with vocab=5 every symbol must carry meaning.
     Control: 1×25 (5×5=25 capacity).

Run:
  PYTHONUNBUFFERED=1 PYTORCH_ENABLE_MPS_FALLBACK=1 python3 _phase54e_pop_simultaneous.py
"""
...
HIDDEN_DIM = 128
DINO_DIM = 384
VOCAB_SIZE = 5       # Changed: 5 levels → 5 symbols
N_HEADS = 2
CONTROL_VOCAB = 25   # Changed: 5×5 = 25
BATCH_SIZE = 32
...
def run_single_seed(seed, data_t, e_bins, f_bins, train_ids, holdout_ids, device):
    ...
    print(f"    Training sender (2×{VOCAB_SIZE}) vs {N_RECEIVERS} receivers "
...
def main():
    print("=" * 70, flush=True)
    print("Phase 54e: Population + Simultaneous Reset + Vocab=5", flush=True)
    print("=" * 70, flush=True)
    ...
    # ════════════════════════════════════════════════════════════
    print(f"\n\n{'='*70}", flush=True)
    print(f"SUMMARY: Phase 54e (3 receivers, simultaneous IL=40, vocab=5)", flush=True)
    print(f"{'='*70}", flush=True)

    header = (f"{'Seed':>6} | {'Holdout Both':>12} | {'PosDis':>7} | "
              f"{'TopSim':>7} | {'MI->e':>7} | {'MI->f':>7} | {'NaN':>4}")
    print(header, flush=True)
    print(f"{'---'*3}-+-{'---'*5}-+-{'---'*3}-+-{'---'*3}-+-{'---'*3}-+-{'---'*3}-+-{'--'*2}",
          flush=True)
    ...
    print(f"  {'Condition':<30} | {'Holdout Both':<16} | {'PosDis':<16} | "
          f"{'MI->e':<14} | {'MI->f':<14}", flush=True)
    print(f"  {'---'*10}-+-{'---'*6}-+-{'---'*6}-+-{'---'*5}-+-{'---'*5}", flush=True)
    ...
    # IL=40 single receiver baseline
    ...
    # 54d staggered population baseline
    pop_hb = [0.882, 0.792, 0.756, 0.834, 0.767]
    pop_pd = [0.228, 0.221, 0.155, 0.666, 0.093]
    pop_me = [0.983, 0.729, 0.909, 1.158, 0.794]
    pop_mf = [0.715, 0.929, 0.821, 1.041, 0.875]

    print(f"  {'IL=40 single rx (54c)':<30} | {fmt(bl_hb, pct=True):<16} | "
          f"{fmt(bl_pd):<16} | {fmt(bl_me):<14} | {fmt(bl_mf):<14}", flush=True)
    print(f"  {'Pop staggered (54d)':<30} | {fmt(pop_hb, pct=True):<16} | "
          f"{fmt(pop_pd):<16} | {fmt(pop_me):<14} | {fmt(pop_mf):<14}", flush=True)
    print(f"  {'Pop simul + vocab=5 (54e)':<30} | {fmt(hb, pct=True):<16} | "
          f"{fmt(pd):<16} | {fmt(me):<14} | {fmt(mf):<14}", flush=True)
    ...
    save_path = RESULTS_DIR / "phase54e_pop_simultaneous.json"
