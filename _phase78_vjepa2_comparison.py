...
=======================================
Same experiment as Phase 54f (2-agent ramp, population IL, 20 seeds, 400 epochs)
but using V-JEPA 2 ViT-L features (1024-dim, mean-pooled) instead of DINOv2
ViT-S/14 features (384-dim × 8 frames).

Since V-JEPA 2 features are already temporally pooled, we use an MLP encoder
(1024→256→128) instead of the temporal Conv1D encoder.

Run:
  PYTHONUNBUFFERED=1 PYTORCH_ENABLE_MPS_FALLBACK=1 python3 _phase78_vjepa2_comparison.py
"""
...
# ══════════════════════════════════════════════════════════════════
# Configuration — identical to Phase 54f except input_dim
# ══════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════
# Architecture — MLP encoder instead of temporal Conv1D
# ══════════════════════════════════════════════════════════════════

class MLPEncoder(nn.Module):
    """MLP encoder for pre-pooled features (V-JEPA 2)."""
...
def train_oracle(data_t, e_bins, f_bins, train_ids, device, seed):
    ...
    oracle = Oracle(MLPEncoder, enc_kwargs, HIDDEN_DIM).to(device)
...
def run_single_seed(seed, data_t, e_bins, f_bins, train_ids, holdout_ids, device):
    ...
    # Sender with MLP encoder
    encoder = MLPEncoder(HIDDEN_DIM, VJEPA_DIM)
...
def main():
    print("=" * 70, flush=True)
    print("Phase 78: V-JEPA 2 backbone comparison", flush=True)
    print("=" * 70, flush=True)
    print(f"  Device: {DEVICE}", flush=True)
    print(f"  Backbone: V-JEPA 2 ViT-L (1024-dim, mean-pooled)", flush=True)
    print(f"  Encoder: MLP (1024→256→128)", flush=True)
    ...
    vjepa_path = str(RESULTS_DIR / "vjepa2_features_pooled.pt")
    data_t, e_bins, f_bins = load_vjepa_features(vjepa_path)
    ...
    # ════════════════════════════════════════════════════════════
    print(f"\n\n{'='*70}", flush=True)
    print(f"FULL TABLE: Phase 78 V-JEPA 2 (400 epochs, 20 seeds)", flush=True)
    ...
    # ════════════════════════════════════════════════════════════
    # Statistical comparison vs Phase 54f (DINOv2)
    # ════════════════════════════════════════════════════════════
    print(f"\n\n{'='*70}", flush=True)
    print(f"COMPARISON: V-JEPA 2 vs DINOv2 (Phase 54f)", flush=True)
    print(f"{'='*70}", flush=True)

    # Load Phase 54f results
    ...
    vjepa_pd = pd
    vjepa_ts = ts
    vjepa_comp = len(compositional)

    print(f"\n  {'Metric':<25} | {'V-JEPA 2':>18} | {'DINOv2':>18}", flush=True)
    print(f"  {'-'*25}-+-{'-'*18}-+-{'-'*18}", flush=True)
    print(f"  {'Holdout both':<25} | {np.mean(vjepa_hb):.1%} ± {np.std(vjepa_hb):.1%}"
          f"{'':>5} | {np.mean(dino_hb):.1%} ± {np.std(dino_hb):.1%}", flush=True)
    print(f"  {'PosDis':<25} | {np.mean(vjepa_pd):.3f} ± {np.std(vjepa_pd):.3f}"
          f"{'':>3} | {np.mean(dino_pd):.3f} ± {np.std(dino_pd):.3f}", flush=True)
    print(f"  {'TopSim':<25} | {np.mean(vjepa_ts):.3f} ± {np.std(vjepa_ts):.3f}"
          f"{'':>3} | {np.mean(dino_ts):.3f} ± {np.std(dino_ts):.3f}", flush=True)
    print(f"  {'Comp rate (PosDis>0.4)':<25} | {vjepa_comp:>2}/20"
          f"{'':>13} | {dino_comp:>2}/20", flush=True)

    # Statistical tests
    t_hb, p_hb = stats.ttest_ind(vjepa_hb, dino_hb)
    t_pd, p_pd = stats.ttest_ind(vjepa_pd, dino_pd)

    d_hb = (np.mean(vjepa_hb) - np.mean(dino_hb)) / np.sqrt(
        (np.std(vjepa_hb)**2 + np.std(dino_hb)**2) / 2)
    d_pd = (np.mean(vjepa_pd) - np.mean(dino_pd)) / np.sqrt(
        (np.std(vjepa_pd)**2 + np.std(dino_pd)**2) / 2)

    print(f"\n  Holdout: t={t_hb:.2f}, p={p_hb:.4f}, Cohen's d={d_hb:.2f}", flush=True)
    print(f"  PosDis:  t={t_pd:.2f}, p={p_pd:.4f}, Cohen's d={d_pd:.2f}", flush=True)

    # 95% CIs
    ci_vjepa = stats.t.interval(0.95, len(vjepa_hb)-1,
                                loc=np.mean(vjepa_hb),
                                scale=stats.sem(vjepa_hb))
    ci_dino = stats.t.interval(0.95, len(dino_hb)-1,
                               loc=np.mean(dino_hb),
                               scale=stats.sem(dino_hb))
    print(f"\n  95% CIs:", flush=True)
    print(f"    V-JEPA 2 holdout: [{ci_vjepa[0]:.1%}, {ci_vjepa[1]:.1%}]", flush=True)
    print(f"    DINOv2 holdout:   [{ci_dino[0]:.1%}, {ci_dino[1]:.1%}]", flush=True)

    # MI matrix for best compositional seed
    if compositional:
        best_comp = max(compositional, key=lambda r: r['pos_dis'])
        print(f"\n  Best compositional seed (V-JEPA 2): seed {best_comp['seed']}", flush=True)
    ...
    save_data = {
        ...
        'comparison_vs_dino': {
            ...
            'posdis_p': float(p_pd),
            'posdis_d': float(d_pd),
            'ci_vjepa': [float(ci_vjepa[0]), float(ci_vjepa[1])],
            'ci_dino': [float(ci_dino[0]), float(ci_dino[1])],
        },
    }

    save_path = RESULTS_DIR / "phase78_vjepa2.json"
